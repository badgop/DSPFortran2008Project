...
!https://stackoverflow.com/questions/6878246/fortran-intrinsic-timing-routines-which-is-better-cpu-time-or-system-clock
    ...
    SUBROUTINE BERTestGFSKVadim (parameterFileName, resultFileName)
          ...
          !Параметры приемника
          CHARACTER(100)                             :: impulseResponseRecieverFileName
          INTEGER(4), ALLOCATABLE , DIMENSION(:)     :: impulseResponseReciever
          ...
          !Выходные файлы
          CHARACTER(100)                             :: outputGFSKSignalFileNameI
          CHARACTER(100)                             :: outputGFSKSignalFileNameQ
          CHARACTER(100)                             :: outputFreqDetectFileName
          ...
          TYPE(GFSKmodulator_t)  ,ALLOCATABLE        :: gfskMod_t
          TYPE(complexSignal_t)                     :: gfskSignal
    ...
    !***** Чтение параметров модели
            ...
            ! приемник
            READ(10,*)  impulseResponseRecieverFileName
            ...
            !Выходные файлы
            READ(10,*)  outputGFSKSignalFileNameI
            READ(10,*)  outputGFSKSignalFileNameQ
            READ(10,*)  outputFreqDetectFileName
...
!            WRITE(*,*)  mIndex
!            WRITE(*,*)  fir_order
!            WRITE(*,*)  capacityFilter
!            WRITE(*,*)  outputFilterShiftGauss
!            WRITE(*,*)  romLengthTruncedInBits
!            WRITE(*,*)  outPutDDSCapacity
!            WRITE(*,*)  centralFrequency
!            !имитатор канала
!            WRITE(*,*)  outPutSampleCapacityChannel
!            ! приемник
!            WRITE(*,*)  impulseResponseRecieverFileName
!            WRITE(*,*)  outPutFilterShiftReciever
!            WRITE(*,*)  decimationRate
!            WRITE(*,*)  outPutFreqShift
!            !Входные файлы
!            WRITE(*,*)  inputNoiseFileName
!            WRITE(*,*)  inputPreambuleFileName
!            !Выходные файлы
!            WRITE(*,*)  outputGFSKSignalFileName
!            WRITE(*,*)  outputFreqDetectFileName
!            WRITE(*,*)  dataFileName
!            WRITE(*,*)  dataWithCRCFileName
!            WRITE(*,*) 'параметры модели прочитаны...'
...
!
             ...
             CALL gfskMod_t%Constructor( baudRate               = baudRate&
                                       ...
                                       , outputFilterShift      = outputFilterShiftGauss&
                                       , capacityFilter         = capacityFilter&
                                       , romLengthTruncedInBits = romLengthTruncedInBits&
                                     )


             gfskSignal = gfskMod_t%Generate(dataArrayHDLC)
             DEALLOCATE (dataArray)

             CALL WriteComplexSignalToFile(gfskSignal,int(2,1),outputGFSKSignalFileNameI,outputGFSKSignalFileNameQ)
!

             ALLOCATE(DemodulatorGFSK)
             ! загрузка ИХ фильтра приемника
             CALL ReadArrayFromFile (impulseResponseReciever,impulseResponseRecieverFileName,'(I12)')

             CALL DemodulatorGFSK%Constructor( decimationRate    = decimationRate&
                                          , impulseResponse   = impulseResponseReciever&
                                          , outputfilterShift = outPutFilterShiftReciever&
                                          , outPutFreqShift   = outPutFreqShift&
                                          )


!           DEALLOCATE(transcieverimpulseResponseReciever)
!
...
!
!           ! загрузка ИХ фильтра приемника
!           CALL ReadArrayFromFile (transcieverimpulseResponseReciever,recievFilterName,'(I12)')
...
!                                              ,impulseResponseRecieverArray    = transcieverimpulseResponseReciever&
